...
- 应收：客户应收汇总按 ARCI 未收、今天至未来 14 天到期、已逾期和逾期超过 60 天汇总，并显示 CI/对应订单下钻；业务概览按计划列出真实到期日，预警金额按币种分开。
...
4. Logo 静态资源为 200。
5. 登录后页面显示数据更新时间、客户订单/供应商订单/CI 三组数量；未来 14 天边界与逾期严格超过 60 天的预警正确，已收清 CI 不出现，客户应收汇总金额与 ARCI 未收合计一致。
