...
月度功能变更后，进入“月度回款计划”并切换过去、当前和未来月份：确认只显示该月到期且截至当前仍未收的活动 ARCI；同一 CI 的分期按各自到期月份出现，已收齐和暂停计划不出现，不同币种分别合计。抽查一个客户的页面明细、分币种合计和英文邮件草稿完全一致；过去月份使用“尽快付款”措辞。缺邮箱时草稿收件人为空，客户关系异常时邮件按钮必须禁用。打开草稿失败或正文较长时，“复制邮件内容”应可作为回退。每次发送前仍要与收款分摊或银行记录核对。
