...
assert.equal(plan[0].items.length, 3);
assert.deepEqual(plan[0].items.filter(item => item.currency === "USD").map(item => item.outstanding), [400, 100]);
assert.deepEqual(plan[0].totals, [["EUR", 50], ["USD", 500]]);
...
assert.match(draft.subject, /Payment Reminder - ACME - June 2026/);
assert.match(draft.body, /Dear Alice/);
assert.match(draft.body, /CI CI-JUNE/);
assert.match(draft.body, /\$500\.00/);
assert.match(draft.body, /€50\.00/);
assert.match(draft.body, /arrange payment by the due date/);
...
assert.match(mixedDraft.body, /both past-due and upcoming/);
